...
class Permission {
  ...
  static checkNotebookPermission(skill, notebookId) {
    ...
    return {
      hasPermission,
      notebookId,
      error: hasPermission ? null : `无权访问笔记本 ${notebookId}`
    };
  }
  ...
  static async checkDocumentPermission(skill, docId) {
    ...
    try {
      ...
      return {
        hasPermission,
        notebookId,
        error: hasPermission ? null : `无权操作文档 ${docId}`
      };
    } catch (error) {
      ...
      if (error.message.includes('tree not found')) {
        if (this.isNotebookId(docId)) {
          ...
          return {
            hasPermission,
            notebookId: docId,
            error: hasPermission ? null : `无权访问笔记本 ${docId}`
          };
        }
  ...
  static async checkParentPermission(skill, parentId, defaultNotebook) {
    ...
    try {
      ...
      return {
        hasPermission,
        notebookId,
        error: hasPermission ? null : `无权在笔记本 ${notebookId} 中操作`
      };
    } catch (error) {
      ...
      if (error.message.includes('tree not found')) {
        if (this.isNotebookId(parentId)) {
          ...
          return {
            hasPermission,
            notebookId: parentId,
            error: hasPermission ? null : `无权在笔记本 ${parentId} 中操作`
          };
        }
        ...
        if (defaultNotebook) {
          ...
          return {
            ...
            error: hasPermission ? null : `无权在笔记本 ${defaultNotebook} 中操作`
          };
        }
