...
**Agent 在写入内容时必须遵守以下规则：**

| 禁止行为 | 原因 | 正确做法 |
|---------|------|----------|
| ❌ 添加 Front Matter | 笔记自身会维护 | 不添加任何 YAML Front Matter |
| ❌ 在正文中重复文档标题 | 标题通过命令参数设置 | 标题通过位置参数设置 |
| ❌ 添加创建/更新时间 | 笔记内部自动处理 | 不在正文中包含时间信息 |
| ❌ 使用标准 Markdown 链接 | 思源使用特殊格式 | 使用 `((id "文本"))` 格式 |

**⚠️ 重要说明：**
- 正文中可以使用 `#` 作为一级标题，这与文档标题不冲突
- 禁止的是在正文开头写 `# 文档标题` 重复文档的标题

**关于标题的说明：**
- ✅ 正文内容中可以包含 Markdown 标题（#、##、### 等）
- ✅ 位置参数 `"标题"` 设置文档的显示标题
- ✅ 正文中的 `# 标题` 是内容的一部分，不影响文档标题

**示例对比：**

```bash
# ❌ 错误做法（包含 Front Matter）
node scripts/create.js "文档" --content '---
title: 我的文档
created: 2024-01-01
---
...
# ✅ 正确做法（包含 Markdown 标题）
node scripts/create.js "我的文档" --content '# 我的文档
