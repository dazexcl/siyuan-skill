...
function checkProtection(docId) {
    const result = ctx.runCmd(`block-attrs ${docId} --get`);
    if (!result.success) return { hasProtect: false, protectValue: null, raw: result.output };
    const output = result.output;
    
    // block-attrs 返回格式: {"success":true,"data":{"attrs":{"custom-protected":"protected"},...}}
    try {
...
// TG-07-01: 临时保护 - 设置并验证标记
{
    ...
    if (!docId) {
        ...
        if (!result.success) {
            ...
            if (protection.hasProtect && protection.protectValue === 'protected') {
...
for (const doc of ctx.createdDocs) {
    ...
    if (protection.protectValue === 'protected') {
