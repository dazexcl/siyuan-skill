...
function runCmd(cmd) {
    ...
    function saveReports(testId, testTitle) {
        ...
        results.forEach(r => {
            ...
            console.log(`| ${r.id} | ${r.name} | \`node scripts/${command}.js ${args}\` | ${r.passed ? '✓ 通过' : '✗ 失败'} | ${details} |`);
        });
...
${results.map(r => {
    const command = r.cmd.split(' ')[0];
    const args = r.cmd.split(' ').slice(1).join(' ');
    return `| ${r.id} | ${r.name} | \`node scripts/${command}.js ${args}\` | ${r.passed ? '✓ 通过' : '✗ 失败'} | ${sanitizeDetails(r.details)} |`;
}).join('\n')}
...
${results.filter(r => !r.passed).map(r => {
    ...
    const args = r.cmd.split(' ').slice(1).join(' ');
    return `### ${r.id}: ${r.name}
- 执行命令: \`node scripts/${command}.js ${args}\`
- 预期结果: ${r.expected}
- 实际结果: ${r.actual}
