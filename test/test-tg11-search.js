...
// TG-11-11: 别名测试 - find
{
    const cmd = `find "机器学习" --notebook ${NOTEBOOK_ID}`;
    const result = runCmd(cmd);
    
    if (!result.success) {
        addResult('TG-11-11', '别名测试 - find', cmd, '与search一致',
            '命令执行失败', false, result.error);
    } else {
        const searchResult = parseSearchResults(result.output);
        if (!searchResult) {
            ...
            addResult('TG-11-11', '别名测试 - find', cmd, '与search一致',
                '成功', true, `find别名工作正常, 结果数: ${searchResult.results.length}`);
        }
    }
}
...
// TG-11-16: 短参数测试 -T (type)
{
    const cmd = `search "算法" -T d --notebook ${NOTEBOOK_ID}`;
    const result = runCmd(cmd);
    
    if (!result.success) {
        addResult('TG-11-16', '短参数 -T (type)', cmd, '与 --type 效果一致',
            '命令执行失败', false, result.error);
    } else {
        const searchResult = parseSearchResults(result.output);
        if (!searchResult) {
            ...
            addResult('TG-11-16', '短参数 -T (type)', cmd, '与 --type 效果一致',
                '成功', true, `-T 别名工作正常, 结果数: ${searchResult.results.length}`);
        }
    }
}

// TG-11-17: 短参数测试 -m (mode)
{
    const cmd = `search "深度学习" -m semantic --notebook ${NOTEBOOK_ID}`;
    const result = runCmd(cmd);
    
    if (!result.success) {
        addResult('TG-11-17', '短参数 -m (mode)', cmd, '与 --mode 效果一致',
            '命令执行失败', false, result.error);
    } else {
        const searchResult = parseSearchResults(result.output);
        if (!searchResult) {
            ...
            const hasScores = searchResult.results.some(r => getScore(r) !== null);
            addResult('TG-11-17', '短参数 -m (mode)', cmd, '与 --mode 效果一致',
                '成功', true, `-m 别名工作正常, 有分数: ${hasScores}`);
        }
    }
}
...
// 保存报告
saveReports('TG-11-search', 'TG-11 搜索功能测试报告');
