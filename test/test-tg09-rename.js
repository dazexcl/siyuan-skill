...
console.log('准备测试数据...');
const testDocTitle = `test_rename_${Date.now()}`;
...
if (testDocId) {
    const dupTitle = `dup_test_${Date.now()}`;
...
// TG-09-03: 强制重命名
if (testDocId) {
    const forceTitle = `force_test_${Date.now()}`;
    ...
    const newForceTitle = `renamed_force_${Date.now()}`;
...
// TG-09-04: 标题斜杠处理
if (testDocId) {
    const slashTitle = `标题/副标题_${Date.now()}`;
