...
// TG-08-03: 别名测试 - mv
{
    const title = `test_mv_alias_${Date.now()}`;
    const parentTitle = `test_mv_parent_${Date.now()}`;
    const createResult = ctx.runCmd(`create "${title}" "内容" --parent-id ${ctx.PARENT_ID}`);
    const parentResult = ctx.runCmd(`create "${parentTitle}" "父文档" --parent-id ${ctx.PARENT_ID}`);
    const docId = ctx.extractDocId(createResult.output);
    const parentId = ctx.extractDocId(parentResult.output);
    
    if (!docId || !parentId) {
        ctx.addResult('TG-08-03', '别名测试 - mv', 'mv <docId> <parentId>', '与move结果一致', '测试文档创建失败', false);
    } else {
        ctx.createdDocs.push({ id: docId, title });
        ctx.createdDocs.push({ id: parentId, title: parentTitle });
        
        const cmd = `mv ${docId} ${parentId}`;
        const result = ctx.runCmd(cmd);
        
        if (!result.success) {
            ctx.addResult('TG-08-03', '别名测试 - mv', cmd, '与move结果一致',
                '命令执行失败', false, result.error || result.output.substring(0, 100));
        } else {
            const hasMoved = result.output.includes('"moved": true') || 
                            result.output.includes('"moved":true') ||
