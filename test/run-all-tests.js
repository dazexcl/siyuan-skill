...
function createRootDoc(title) {
    const scriptPath = path.join(SIYUAN_SKILL_DIR, 'scripts', 'create.js');
    const cmd = `node "${scriptPath}" "${title}" "测试运行根文档" --parent-id ${NOTEBOOK_ID}`;
    const output = execSync(cmd, { encoding: 'utf-8', cwd: SIYUAN_SKILL_DIR, shell: true, stdio: ['ignore', 'pipe', 'pipe'] });
...
function deleteRootDoc(docId, title) {
    const scriptPath = path.join(SIYUAN_SKILL_DIR, 'scripts', 'delete.js');
    const cmd = `node "${scriptPath}" ${docId} --confirm-title "${title}"`;
    execSync(cmd, { encoding: 'utf-8', cwd: SIYUAN_SKILL_DIR, shell: true, stdio: ['ignore', 'pipe', 'pipe'] });
}
...
async function main() {
    ...
    const testFiles = fs.readdirSync(TEST_DIR)
        ...
        .filter(f => !f.includes('test-tg00-global'))  // 排除 TG-00 统一入口测试
        .sort();
