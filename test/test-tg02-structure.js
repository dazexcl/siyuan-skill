...
// TG-02-05: 别名 ls 测试
{
    const cmd = `ls ${ctx.NOTEBOOK_ID} --depth -1`;
    const result = ctx.runCmd(cmd);
    const structure = parseStructure(result.output);
    
    if (!result.success) {
        ctx.addResult('TG-02-05', '别名测试 - ls', cmd, '与structure结果一致', 
            '命令执行失败', false, result.error);
    } else if (!structure) {
        ctx.addResult('TG-02-05', '别名测试 - ls', cmd, '与structure结果一致', 
            '无法解析返回数据', false, result.output.substring(0, 200));
        ...
        if (rootDoc && rootDoc.title === rootTitle) {
            ctx.addResult('TG-02-05', '别名测试 - ls', cmd, '与structure结果一致', 
                '成功', true, 'ls别名返回相同结构');
        } else {
            ctx.addResult('TG-02-05', '别名测试 - ls', cmd, '与structure结果一致', 
                '结果不一致', false, 'ls未返回相同的文档结构');
        }
    }
}
...
// TG-02-07: 短参数测试 -d (depth)
{
    const cmd = `structure ${ctx.NOTEBOOK_ID} -d -1`;
    const result = ctx.runCmd(cmd);
    const structure = parseStructure(result.output);
    
    if (!result.success) {
        ctx.addResult('TG-02-07', '短参数 -d (depth)', cmd, '与 --depth 效果一致', 
            '命令执行失败', false, result.error);
    } else if (!structure) {
        ctx.addResult('TG-02-07', '短参数 -d (depth)', cmd, '与 --depth 效果一致', 
            '无法解析结果', false);
        ...
        if (foundCount >= 4) {
            ctx.addResult('TG-02-07', '短参数 -d (depth)', cmd, '与 --depth 效果一致', 
                '成功', true, `-d 别名工作正常，找到 ${foundCount} 个文档`);
        } else {
            ctx.addResult('TG-02-07', '短参数 -d (depth)', cmd, '与 --depth 效果一致', 
                '深度限制未生效', false, `只找到 ${foundCount} 个文档`);
        }
    }
}

ctx.saveReports('TG-02-structure', 'TG-02 文档结构测试报告');
