...
// TG-06-03: 别名测试 - rm
{
    const title = `test_delete_rm_${Date.now()}`;
    const createResult = ctx.runCmd(`create "${title}" "rm别名测试" --parent-id ${ctx.PARENT_ID}`);
    const docId = ctx.extractDocId(createResult.output);
    
    if (!docId) {
        ctx.addResult('TG-06-03', '别名测试 - rm', 'rm <docId>', '与delete一致', '测试文档创建失败', false);
    } else {
        const cmd = `rm ${docId} --confirm-title "${title}"`;
        const result = ctx.runCmd(cmd);
        
        if (!result.success) {
            ctx.addResult('TG-06-03', '别名测试 - rm', cmd, '与delete一致',
                '删除命令失败', false, result.error || result.output.substring(0, 100));
        } else {
            ctx.sleep(500);
            const stillExists = checkDocExists(docId);
...
// TG-06-05: 标题不匹配拒绝删除
{
    ...
    if (!docId) {
        ...
        const rejected = !result.success || 
                        result.output.includes('不匹配') ||
                        result.output.includes('拒绝') ||
                        result.output.includes('mismatch') ||
                        result.output.includes('reject');
