...
class VectorManager {
      ...
      try {
        const points = await Promise.all(
          batch.map(async (doc) => {
            ...
            return {
              id: this.hashStringToUint(doc.docId),
              vector: {
                dense: denseVector,
                sparse: sparseVector
              },
    ...
    try {
      ...
      while (hasMore) {
        ...
        for (const point of points) {
          const blockId = point.payload?.block_id || '';
          const originalDocId = point.payload?.original_doc_id || '';
          if (docIdSet.has(blockId) || docIdSet.has(originalDocId)) {
            const matchedId = docIdSet.has(blockId) ? blockId : originalDocId;
            const existingTime = updateTimes.get(matchedId) || 0;
            const pointTime = point.payload?.updated || 0;
            updateTimes.set(matchedId, Math.max(existingTime, pointTime));
          }
        }

        nextPageOffset = result.result?.next_page_offset;
        hasMore = !!nextPageOffset && points.length === batchSize;
      }
    } catch (error) {
  ...
  /**
   * 将字符串哈希为无符号 32 位整数（用于将 Siyuan 文档 ID 转为 Qdrant 可接受的数字 ID）
